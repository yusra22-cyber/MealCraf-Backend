...
async function handlePantrySuggestion(req,res){
    try {
        ...
        const ingedients = user.map(item => pluralize.singular(item.ingredientName.toLowerCase()))
        ...
        const systemItems = systemIngredients.map(item=>{
         ...
         const  matched =  item.ingredients.filter(i=>
                (ingedients.includes(pluralize.singular(i.ingredientName.toLowerCase())))).map(i=>i.ingredientName)
            ////  missing items
